...
double WHEEL_RADIUS = 1.625; //3.25/2
...
//The starting x and y coordinates of the bot (INCHES)
  ...
  //Relative to: MIDDLE
double X_START = -40; //19.1
double Y_START = 40; //8.5

//Distances of tracking wheels from tracking center (INCHES)
double LTrackRadius = 0; //was 6.87
double RTrackRadius = 0; //6.8335
double STrackRadius = 11.25;
...
int positionTracking() {
  while(1) {
    ...
    LPos = TW_front.get_value();
    RPos = TW_front.get_value();
    SPos = TW_side.get_value();
    ...
    currentAbsoluteOrientation = (360 - Gyro.get_heading()) * M_PI / 180.0;
